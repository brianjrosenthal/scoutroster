...
try {
    ...
    // Assign the position using LeadershipManagement
    LeadershipManagement::assignPackPosition($ctx, $adultId, $positionId);
    
    echo json_encode([
        'ok' => true,
        'message' => "$adultName has been assigned to {$position['name']}"
    ]);
    ...
    error_log('Error in position_assign_ajax.php: ' . $e->getMessage());
