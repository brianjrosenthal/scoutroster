...
try {
    ...
    // Remove the den leader position using LeadershipManagement
    LeadershipManagement::removeDenLeader($ctx, $adultId, $grade);
    
    $response = [
        'ok' => true,
        'message' => "$adultName has been removed as den leader for $gradeLabel",
        'adult_id' => $adultId,
        'grade' => $grade
    ];
    ...
    error_log('Error in den_leader_remove_by_grade_ajax.php: ' . $e->getMessage());
    $response = ['ok' => false, 'error' => 'An error occurred while removing den leader'];
}
