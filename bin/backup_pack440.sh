...
create_backup() {
    ...
    echo "$mysqldump_cmd > $temp_file 2>/dev/null"
...
cleanup_old_backups() {
    ...
    for backup_file in "$BACKUP_DIR"/pack440_*.sql.gz; do
        ...
        # Convert date to timestamp (compatible with both macOS and Linux)
        local file_timestamp
        if [[ "$OSTYPE" == "darwin"* ]]; then
            # macOS uses BSD date
            file_timestamp=$(date -j -f "%Y-%m-%d" "$date_part" '+%s' 2>/dev/null || echo "0")
        else
            # Linux uses GNU date
